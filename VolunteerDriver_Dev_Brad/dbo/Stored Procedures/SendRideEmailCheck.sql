...
-- =============================================-- exec SendRideEmail 
--exec sendrideemailcheck 20306
CREATE PROCEDURE [dbo].[SendRideEmailCheck](@trip_number int)
as
...
PUFacility varchar(100),
DOFacility varchar(100),
clAltPhone1 varchar(50),
clAltPhone2 varchar(50)
)
...
, 	(Select isnull(FacilityName,'') from tblAddress where AddressID = PUAddressID)	as PUFacility
, 	(Select isnull(FacilityName,'') from tblAddress where AddressID = DOAddressID)	as DOFacility
, tblClient.clAltPhone1
, tblClient.clAltPhone2
...
--Update @InitQuery set clAltPhone1 = ISNULL(clAltPhone1,'')
update @InitQuery set clAltPhone1 = REPLACE(clAltPhone1,'-','') 
update @InitQuery set clAltPhone1 = REPLACE(clAltPhone1,')','')
update @InitQuery set clAltPhone1 = REPLACE(clAltPhone1,'(','')
update @InitQuery set clAltPhone1 = REPLACE(clAltPhone1,'/','')
update @InitQuery set clAltPhone1 = REPLACE(clAltPhone1,' ','')
--Add - to 7 digit numbers
update @InitQuery set clAltPhone1 = (substring(clAltPhone1,1,3) + '-' + substring(clAltPhone1,4,4)) where LEN(clAltPhone1) = 7
--Add - to 10 digit numbers
update @InitQuery set clAltPhone1 = (substring(clAltPhone1,1,3) + '-' + 
substring(clAltPhone1,4,3) + '-' + SUBSTRING(clAltPhone1,7,4)) where LEN(clAltPhone1) = 10
--Replace invalid numbers with warning text
update @InitQuery set clAltPhone1 = 'Invalid phone number' where LEN(clAltPhone1) <> 8 and LEN(clAltPhone1) <> 12

update @InitQuery set clAltPhone2 = REPLACE(clAltPhone2,'-','') 
update @InitQuery set clAltPhone2 = REPLACE(clAltPhone2,')','')
update @InitQuery set clAltPhone2 = REPLACE(clAltPhone2,'(','')
update @InitQuery set clAltPhone2 = REPLACE(clAltPhone2,'/','')
update @InitQuery set clAltPhone2 = REPLACE(clAltPhone2,' ','')
--Add - to 7 digit numbers
update @InitQuery set clAltPhone2 = (substring(clAltPhone2,1,3) + '-' + substring(clAltPhone2,4,4)) where LEN(clAltPhone2) = 7
--Add - to 10 digit numbers
update @InitQuery set clAltPhone2 = (substring(clAltPhone2,1,3) + '-' + 
substring(clAltPhone2,4,3) + '-' + SUBSTRING(clAltPhone2,7,4)) where LEN(clAltPhone2) = 10
--Replace invalid numbers with warning text
update @InitQuery set clAltPhone2 = 'Invalid phone number' where LEN(clAltPhone2) <> 8 and LEN(clAltPhone2) <> 12
/*
...
*/
-- exec sendrideemailchecktext 20703
...
SET @XMLQueryResults = 					
    CAST ( ( SELECT td = coalesce(RideDate,'No Ride Date'),    '',
				 'color:red' as 'td/@style', td = coalesce(ScheduleApptTime, 'No Appointment time'), '',
                    ...
                    td = case when PUFacility in ('','-',' -', ' ', ' - ','- ') then '' else coalesce(PUFacility,'-') + ', ' end + coalesce(RPickUpAddress,'No address') + ', ' + coalesce(RPickupCity,'No city'), '',
                    --td = coalesce(RPickupCity,'No pickup city'), '',
                    td = case when DOFacility in ('','-',' -', ' ', ' - ','- ') then '' else coalesce(DOFacility,'-') + ', ' end + coalesce(RDropoffAddress,'No address') + ', ' + coalesce(RDropoffCity,'No City'), '',
                    ...
                    --CASE WHEN (Select COUNT(*) from @InitQuery where RideNotes is not null) > 0  then coalesce(RideNotes, 'No Ride Notes' ) + coalesce(clAltPhone1,' ') + coalesce(clAltPhone2,' ')  END as td, '',
                    CASE WHEN RideNotes is not null and clAltPhone1 is null and clAltPhone2 is null   then coalesce(RideNotes, '' ) 
                         WHEN RideNotes is not null and (clAltPhone1 is not null or clAltPhone2 is not null)   then coalesce(RideNotes, '' ) + ' Alt Phone # ' + coalesce(clAltPhone1,' ') + coalesce(clAltPhone2,' ')
                         When RideNotes is null and (clAltPhone1 is not null or clAltPhone2 is not null) then 'Alt Phone # ' + coalesce(clAltPhone1,' ') + coalesce(clAltPhone2,' ') END as td, '',
