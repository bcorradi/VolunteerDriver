...
CREATE procedure [dbo].[VolunteerEZClaimInsert]  (@insuranceid int, @startdate datetime, @enddate datetime)
as
set nocount on
...
drivermeals+ drivermisc as MealsMisc,
	...
	(
		...
		case when @insuranceid not in (29,2) And len(CAST(RepID as varchar(25))) < 5 then 'Verify RepID. ' else '' end +
...
max(	(
		...
		case when @insuranceid not in (29,2) And len(CAST(r.RepID as varchar(25))) < 5 then 'Verify RepID. ' else '' end +
