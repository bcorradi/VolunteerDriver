...
-- =============================================-- exec SendRideEmail
CREATE PROCEDURE [dbo].[SendRideEmailCheck2](@DriverID int)
as
...
--DECLARE @XMLQueryResults NVARCHAR(MAX)
 DECLARE @XMLQueryResults NVARCHAR(max) 
DECLARE @XMLQueryResults2 NVARCHAR(MAX)
...
WHERE (((tblride.SeqNo) = 1 and cast(tblride.RideDate as DATE) >= cast(GETDATE() as Date) and (tblride.driverID)=@DriverID and (tblRide.Canceled = 0 or tblride.Canceled = null)) )
order by tblride.RideDate,isnull(tblride.SchedulePickupTime,tblride.ScheduleApptTime)
