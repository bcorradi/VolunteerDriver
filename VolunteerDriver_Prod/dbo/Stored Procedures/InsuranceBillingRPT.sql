CREATE procedure [dbo].[InsuranceBillingRPT]  (@insuranceid int, @startdate datetime, @enddate datetime)
as
set nocount on
...
case when @tripfeesTrue <> 0 and PendingPaperwork <> 'P' and minofridedate < 5/1/19 then(case when max(legs) = 1 then 10.00 else 20.00 end)
	 when @tripfeesTrue <> 0 and PendingPaperwork <> 'P' and minofridedate >= 5/1/19 then sum(legs*10) --(case when legs = 1 then 10.00 else 20.00 end) else 0 end as TripFee,
...
cast(round(sum	(case when loadedmilesbill = 0 then 0 
	else ISNULL(case when @tripfeesTrue <> 0 then (case when legs = 1 then 10.00 else legs*10.00 end) else 0 end,0)
	end + --Trip Fee Billed
...
noshow,
insurance,
case when @tripfeesTrue <> 0 and PendingPaperwork <> 'P' then(case when legs = 1 then 10.00 else 20.00 end) else 0 end,
