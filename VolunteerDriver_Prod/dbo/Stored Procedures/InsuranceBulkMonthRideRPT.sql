...
CREATE procedure [dbo].[InsuranceBulkMonthRideRPT] 
...
,@startdate datetime, @enddate datetime, @insuranceid int        
set @startdate = '2018-1-01'
set @enddate = '2019-3-30'
set @insuranceid = 20
-------------------------------------------------------------------------------------------
...
,count(distinct (case when t.tripcanceled = 0 and t.MIPOnly <> 'true' and t.TripTypeId<>6 then t.tripid end)) as Trips
...
,count((case when r.noshow = 'true' then r.rideid end)) as NoShowLegs
...
,count(distinct (case when t.tripcanceled <> 0 then t.tripid end)) as Cancels
...
GROUP BY i.Insurance, Year([RideDate]), Month([RideDate])
ORDER BY i.Insurance, Year([RideDate]), Month([RideDate])
